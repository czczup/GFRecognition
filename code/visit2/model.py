...
class VisitModel(object):
    def __init__(self):
        with tf.name_scope("image"):
            self.image = tf.placeholder(tf.float32, [None, 182, 24, 24], name='image')
    ...
    def model(self, image):
        ...
        image = tf.nn.dropout(image, self.keep_prob)
        channel = 26
        with tf.name_scope("Resnet"):
            with tf.name_scope('stage1'):
                conv = self.conv2d(image, 182, channel, 7, 1)
            ...
            with tf.name_scope('stage5'):
                ...
                print(res)
                pool = tf.nn.avg_pool(res, [1, 3, 3, 1], strides=[1, 1, 1, 1], padding='VALID')
                print(pool)
